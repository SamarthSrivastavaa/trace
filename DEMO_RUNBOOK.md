...
python -m pytest tests/ -q          # preflight: expect "391 passed"
```
...
```
COMMAND:
python -m pramaan --db demo.db demo

EXPECTED RESULT:
MODE   state=SIMULATED model=MOCK
db     demo.db
policy pramaan-default@v1+0944fd4b52f21acf
...
```
COMMAND:
python -m pramaan --db demo.db recheck --all
...
| `recheck --all` says `no proofs` | `demo.db` never created, or a `/tmp` path was used | `rm -f demo.db && python -m pramaan --db demo.db demo` | No |
...
|---|---|---|
| "The deterministic core is exercised offline by fixtures and 391 tests." | "391 tests prove the system works in production." | No live path is exercised by any test. |
...
| "Razorpay acquisition is a boundary skeleton and is not demonstrated live." | "Integrated with Razorpay." | 8 of 12 field mappings are unresolved TODOs; `get_state` raises. |
...
| "The model cannot represent a verdict — `ProposedClaim` has no status field." | "The AI decides whether to send." | Verifiable in `pramaan/core/verify/schema.py`. |
